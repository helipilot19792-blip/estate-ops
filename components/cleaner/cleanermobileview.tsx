...
export default function CleanerMobileView({
  ...
  function filterBySelectedDate(items: CleanerJob[]) {
    if (!selectedDate) return items;
    return items.filter((item) => normalizeJobDate(item.jobDate) === selectedDate);
  }

  function renderJobCard(item: CleanerJob) {
  ...
  const activeJobsSafe = activeJobs ?? cleanerJobs;
  const historyJobsSafe = historyJobs ?? [];
  const urgentJobs = activeJobsSafe.filter((item) => isOffered(item.slot.status));
  const activeNonUrgentJobs = activeJobsSafe.filter((item) => !isOffered(item.slot.status));

  const visibleUrgentJobs = filterBySelectedDate(urgentJobs);
  const visibleActiveJobs = filterBySelectedDate(activeNonUrgentJobs);
  const visibleHistoryJobs = filterBySelectedDate(historyJobsSafe);
  ...
  const selectedHasAnyJobs =
    visibleUrgentJobs.length > 0 || visibleActiveJobs.length > 0 || visibleHistoryJobs.length > 0;
  ...
  return (
    <main className="min-h-screen bg-[#0f0d0a] px-3 py-4 text-[#f5efe4]">
      <div className="mx-auto max-w-md space-y-4">
        ...
        {urgentJobs.length > 0 ? (
          <section className="rounded-2xl border border-red-400/60 bg-red-600 p-4 text-white shadow-[0_0_28px_rgba(239,68,68,0.28)]">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-base font-semibold">
                  {urgentJobs.length} urgent job{urgentJobs.length === 1 ? "" : "s"} waiting for
                  your response
                </p>
              </div>
        ...
        <div className="flex gap-2 overflow-x-auto pb-1">
          {dateStrip.map((ymd) => {
            const jobsForDay = activeJobsSafe.filter((item) => normalizeJobDate(item.jobDate) === ymd);
            const hasOffered = jobsForDay.some((item) => isOffered(item.slot.status));
            const allAccepted =
              jobsForDay.length > 0 &&
              jobsForDay.every((item) => (item.slot.status || "").toLowerCase().trim() === "accepted");
        ...
        {!selectedHasAnyJobs ? (
          <div className="rounded-2xl border border-[#7a5c2e]/20 bg-[#15110d] p-4 text-sm text-[#cdbda0]">
            {selectedDate ? "No jobs for that date." : "No active jobs assigned right now."}
          </div>
        ) : null}

        {visibleUrgentJobs.length > 0 ? (
          <section className="space-y-3">
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-semibold uppercase tracking-[0.18em] text-red-300">
                Urgent Jobs
              </h2>
              <span className="rounded-full border border-red-500/40 bg-red-500/15 px-2.5 py-1 text-[11px] text-red-200">
                {visibleUrgentJobs.length}
              </span>
            </div>
            {visibleUrgentJobs.map(renderJobCard)}
          </section>
        ) : null}
        ...
        {visibleActiveJobs.length > 0 ? (
          <section className="space-y-3">
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-semibold uppercase tracking-[0.18em] text-[#e7c98a]">
                Active Jobs
              </h2>
              <span className="rounded-full border border-[#7a5c2e]/30 bg-[#15110d] px-2.5 py-1 text-[11px] text-[#d4c4a8]">
                {visibleActiveJobs.length}
              </span>
            </div>
            {visibleActiveJobs.map(renderJobCard)}
          </section>
        ) : null}

        {visibleHistoryJobs.length > 0 ? (
          <section className="space-y-3 border-t border-[#7a5c2e]/20 pt-2">
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-semibold uppercase tracking-[0.18em] text-[#9f9178]">
                History
              </h2>
              <span className="rounded-full border border-[#7a5c2e]/20 bg-[#120f0b] px-2.5 py-1 text-[11px] text-[#9f9178]">
                {visibleHistoryJobs.length}
              </span>
            </div>
            {visibleHistoryJobs.map(renderJobCard)}
          </section>
        ) : null}
      </div>
    </main>
