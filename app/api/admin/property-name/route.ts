...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (propertyName.length > 160) {
      return NextResponse.json({ error: "Property name must be 160 characters or fewer." }, { status: 400 });
    }
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Could not save property name." },
      { status: 500 }
    );
  }
}
